...
def create_csv(): # ALL GOOD
    ...
    songData_C = pandas.DataFrame({ 'name': ['C Major Scale'], 
    ...
    songData_D = pandas.DataFrame({ 'name': ['D Major Scale'], 
                                    'data': [['C4',0.25,1,  'D4',0.25,2,  'E4',0.25,3,  'F4',0.25,1,  'G4',0.25,2,  'A4',0.25,3,  'B4',0.25,4,  'C5',0.25,5,
                                              'B4',0.25,4,  'A4',0.25,3,  'G4',0.25,2,  'F4',0.25,1,  'E4',0.25,3,  'D4',0.25,2,  'C4',0.25,1]]} )
    ...
    allData.to_csv(os.path.realpath(os.path.join(os.path.dirname(__file__), '..', 'song_gallery', 'song_gallery.csv')),mode='w',index=True,header=headerList)
...
def append_csv(name, data): # good (i think)
    ...
    newData.to_csv(os.path.realpath(os.path.join(os.path.dirname(__file__), '..', 'song_gallery', 'song_gallery.csv')),mode='a',index=True, header=False) # append new data to csv
...
def song_gallery(name, data, action): # ALL GOOD
    ...
    path = os.path.realpath(os.path.join(os.path.dirname(__file__), '..', 'song_gallery', 'song_gallery.csv'))
    ...
    if (action == "read"):
        ...
        allData = pandas.read_csv(os.path.realpath(os.path.join(os.path.dirname(__file__), '..', 'song_gallery', 'song_gallery.csv')),usecols=['data']) # copy dataframe from csv
        ...
        songData = pandas.read_csv(os.path.realpath(os.path.join(os.path.dirname(__file__), '..', 'song_gallery', 'song_gallery.csv')),usecols=['data'],skiprows=skip_rows_list) # create dataframe for desired song
