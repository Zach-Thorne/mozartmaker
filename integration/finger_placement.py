...
def fingers(song, note_array):
    
    finger_array = []
    #print("note array ", len(note_array[0]))
    ...
    for p in range(1, len(finger_array)):
        ...
        if (p > 7): # if the values get changed with the 5, doesn't reallllly matter cuz this will overwrite those changes if necessary so whatever
            if ((finger_array[p]>finger_array[p-1]) & (finger_array[p-1]>finger_array[p-2]) & (finger_array[p-2]>finger_array[p-3]) & (finger_array[p-3]>finger_array[p-4]) & (finger_array[p-4]>finger_array[p-5]) & (finger_array[p-5]>finger_array[p-6]) & (finger_array[p-6]>finger_array[p-7]) & (finger_array[p-7]>finger_array[p-8])):       
                # fix solution 1
                sol1[p-8] = 1
                sol1[p-7] = 2
                sol1[p-6] = 3
        ...
        if (p < (len(finger_array)-1)):
            semitones_1_fwd = finger_array[p+1] - finger_array[p]
        if (p < (len(finger_array)-2)):
            semitones_2_fwd = finger_array[p+2] - finger_array[p]
        if (p < (len(finger_array)-3)):
            semitones_3_fwd = finger_array[p+3] - finger_array[p]
        ...
        if (p > dont_touch_index):
            for f2 in range(1,6): # iterates through values 1 to 5
                ...
                if (f2>3):
                    if (p<(len(finger_array)-2)):
                        if ((semitones > 0) & (finger_array[p+2] > finger_array[p])): # if the notes continue increasing
